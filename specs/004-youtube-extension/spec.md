...
- Q: 當使用者透過 LINE OAuth 成功驗證，但系統中找不到該 LINE 帳號的註冊資料時，擴充程式應該如何處理？ → A: 顯示歡迎訊息並自動建立新會員帳號，讓使用者立即開始使用
- Q: Access token 與 refresh token 的有效期限各是多長？ → A: Access token: 1 小時，Refresh token: 7 天
- Q: 播放庫和播放清單是否有影片數量上限？ → A: 播放庫與播放清單都無數量限制
- Q: 擴充程式在 YouTube 網站上的權限範圍應該是什麼？ → A: 僅在使用者點擊擴充程式圖示時執行，讀取當前分頁的 URL
- Q: 當使用者將 YouTube 影片加入播放庫或播放清單時，系統應該儲存哪些影片資訊？ → A: 儲存影片 ID、標題、縮圖 URL、影片時長
...
3. **Given** 使用者已通過 LINE 驗證且該 LINE 帳號已在系統註冊，**When** 系統檢查會員資料，**Then** 顯示「加入播放庫」與「加入播放清單」按鈕
4. **Given** 使用者已通過 LINE 驗證但該 LINE 帳號尚未註冊，**When** 系統檢查會員資料，**Then** 顯示歡迎訊息並自動建立新會員帳號，然後顯示「加入播放庫」按鈕（新會員尚無播放清單）
5. **Given** 使用者已登入且 token 有效，**When** 重新開啟擴充程式，**Then** 自動使用儲存的 token 維持登入狀態
6. **Given** 使用者的 access token 過期，**When** 擴充程式偵測到 token 過期，**Then** 使用 refresh token 自動更新 access token
...
- **FR-002**: 擴充程式必須僅在使用者點擊擴充程式圖示時執行，讀取當前分頁的 YouTube URL，不在背景自動執行或監控
- **FR-003**: 擴充程式必須提供 LINE OAuth 2.0 登入功能
- **FR-004**: 擴充程式必須儲存並管理 access token（有效期 1 小時）與 refresh token（有效期 7 天），遵循與現有系統相同的 token 管理規則
- **FR-005**: 擴充程式必須在 access token 過期（1 小時後）時，自動使用 refresh token 更新 access token
- **FR-006**: 擴充程式必須在 refresh token 過期（7 天後）時，提示使用者重新登入
- **FR-007**: 擴充程式必須在使用者登入後，向後端 API 驗證該 LINE 帳號是否已註冊，若未註冊則顯示歡迎訊息並自動建立新會員帳號
- **FR-008**: 擴充程式必須在 YouTube 影片頁面偵測並解析當前影片的 URL，提取影片 ID
- **FR-009**: 擴充程式必須在解析 YouTube URL 時，忽略播放清單參數（list），僅提取單一影片 ID
- **FR-010**: 擴充程式必須提供「加入播放庫」按鈕，將當前影片的 ID、標題、縮圖 URL、時長加入使用者的播放庫
- **FR-011**: 擴充程式必須在影片已存在於播放庫或播放清單時，顯示提示訊息避免重複加入
- **FR-012**: 擴充程式必須在使用者沒有任何播放清單時，隱藏「加入播放清單」按鈕
- **FR-013**: 擴充程式必須提供「加入播放清單」按鈕，支援預設模式與自訂模式
- **FR-014**: 擴充程式在預設模式下，必須將影片加入使用者指定的預設播放清單，若未指定則使用第一個播放清單
- **FR-015**: 擴充程式在自訂模式下，必須彈出播放清單選單供使用者選擇
- **FR-016**: 擴充程式必須提供設定介面，讓使用者選擇「預設模式」或「自訂模式」
- **FR-017**: 擴充程式在預設模式下，必須提供播放清單選單讓使用者設定預設播放清單
- **FR-018**: 擴充程式必須在非 YouTube 頁面或非影片頁面時，顯示適當的提示訊息
- **FR-019**: 擴充程式必須在網路錯誤或 API 失敗時，顯示錯誤訊息並提供重試選項

### Key Entities

- **使用者帳號**: 透過 LINE OAuth 驗證的會員，擁有唯一的 LINE User ID，對應系統中的會員資料
- **Token 資訊**: 包含 access token（有效期 1 小時）與 refresh token（有效期 7 天），用於身份驗證與授權
- **播放庫**: 使用者收藏的單一影片集合，每部影片包含 YouTube 影片 ID、標題、縮圖 URL、時長，無數量限制
- **播放清單**: 使用者建立的影片分類清單，可包含多部影片，每個播放清單有名稱與唯一 ID，無數量限制
- **影片資訊**: 從 YouTube 擷取的影片資料，包含影片 ID（唯一識別）、標題、縮圖 URL、影片時長
...
- 假設後端 API 已提供會員建立、查詢播放庫與播放清單、新增影片的 RESTful API
